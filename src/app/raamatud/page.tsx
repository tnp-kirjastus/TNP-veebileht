...
import { filterProductsByPerson, getActiveProducts, searchProducts, getCategories, getProductsByCategory, isOnSale, getSalePercent, type Product } from "@/lib/data";

export const revalidate = 3600;

interface SearchParams {
  q?: string; category?: string; origin?: string; sale?: string; upcoming?: string;
...
const CATEGORY_TREE = getCategories().map((category, index) => ({
  id: String(index + 1), slug: category.slug, name_et: category.name,
}));

function mapProduct(p: Product) {
  const onSale = isOnSale(p);
  return { slug: p.slug, title: p.title_et, author: p.people.author?.join(", ") || "", price: p.price, salePrice: p.sale_price, effectivePrice: onSale ? p.sale_price! : p.price, coverImage: p.cover_image, isUpcoming: p.is_upcoming, isOnSale: onSale, salePercent: getSalePercent(p) };
}
...
function BooksContent({ params }: { params: SearchParams }) {
  let results = params.q?.trim() ? searchProducts(params.q.trim()) : getActiveProducts();

  if (params.category) {
    const allowed = new Set(getProductsByCategory(params.category).map((product) => product.id));
    results = results.filter((product) => allowed.has(product.id));
  }
  ...
  const activeLabel = params.q ? `Otsing: "${params.q}"`
    : params.category ? CATEGORY_TREE.find(c => c.slug === params.category)?.name_et || params.category
    : t.books.title;

  return (
    <LayoutFull>
      <section className="py-[48px] pb-[60px] border-b border-line">
        <Shell>
          <Breadcrumbs crumbs={[{ label: "Esileht", href: "/" }, { label: activeLabel }]} />
          <div className="flex justify-between items-end gap-6 max-[640px]:flex-col max-[640px]:items-start">
            <h1 className="font-heading text-[clamp(42px,7vw,78px)] leading-none">{params.q ? `Otsing: "${params.q}"` : activeLabel}</h1>
            <p className="max-w-[620px] text-muted">{t.books.browse_all}</p>
          </div>
        </Shell>
      </section>
      ...
      <Shell>
        <div className="grid grid-cols-[260px_1fr] gap-[38px] pt-8 pb-10 max-[880px]:grid-cols-1">
          <aside className="self-start sticky top-[116px] max-[880px]:relative max-[880px]:top-0">
            <FilterSidebar categories={CATEGORY_TREE} activeCategory={params.category} currentParams={params as Record<string, string | undefined>} />
          </aside>
          <section>
