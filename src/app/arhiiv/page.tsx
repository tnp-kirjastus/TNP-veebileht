...
import { searchCatalogue } from "@/lib/db/catalog";
...
export default async function ArchivePage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string }> }) {
  ...
  const result = await searchCatalogue({ q: query || undefined, scope: "archived", sort: "az", page, pageSize: 48 });
  ...
  return <LayoutFull>
    ...
    <Shell><div className="flex justify-between gap-4 py-8"><p className="text-muted">{result.totalCount} raamatut</p>{query && <Link href="/arhiiv" className="font-bold text-accent">Tühista otsing</Link>}</div>
