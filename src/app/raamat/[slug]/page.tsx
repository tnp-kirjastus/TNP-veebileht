...
import { getCategories, getPeople, getProductBySlug, getRelatedProducts, getSameSeriesProducts, getSameAuthorProducts } from "@/lib/db/catalog";
import { isOnSale, formatEuro, formatDate, formatEditions } from "@/lib/product-utils";
...
function personHref(slugByName: Map<string, string>, role: string, name: string) {
  const value = slugByName.get(name) ?? name;
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [storeSettings, categories, people, relatedCat, sameAuthor, sameSeries] = await Promise.all([
    getStoreSettings(),
    getCategories(),
    getPeople(),
    getRelatedProducts(product, 5),
    getSameAuthorProducts(product, 5),
    getSameSeriesProducts(product, 5),
  ]);
  ...
  const relatedCards = relatedCat.map(mapProduct);
  ...
  return (
    <LayoutContained>
      ...
      {relatedCards.length > 0 && (
        <section className="py-[50px] pb-[10px]">
          <div className="flex items-end justify-between gap-5 mb-[22px]">
            <h2 className="font-heading text-[28px]">{product.categories[0] ? "Samast kategooriast" : "Sellest kategooriast"}</h2>
      ...
      {sameAuthorCards.length > 0 && primaryAuthor && (
        <section className="py-[50px] pb-[10px]">
          <div className="flex items-end justify-between gap-5 mb-[22px]">
            <h2 className="font-heading text-[28px]">Samalt autorilt: {primaryAuthor}</h2>
            <Link href={personHref(slugByName, "author", primaryAuthor)} className="text-accent font-extrabold text-sm hover:text-accent-dark transition-colors">Kõik autori raamatud →</Link>
          </div>
          <ProductGrid products={sameAuthorCards} columns={5} />
        </section>
      )}

      {sameSeriesCards.length > 0 && product.series_slug && (
        <section className="py-[50px] pb-[10px]">
          <div className="flex items-end justify-between gap-5 mb-[22px]">
          ...
          <ProductGrid products={sameSeriesCards} columns={5} />
        </section>
      )}
