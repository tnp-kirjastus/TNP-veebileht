...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> { const locale = (await params).locale; if (locale !== "et") return { robots: { index: false, follow: false } }; return { title: content.et.title, description: content.et.subtitle, alternates: { canonical: "/et/kontakt" } }; }

export default async function LocalizedContactPage({ params }: { params: Promise<{ locale: string }> }) {
  const locale = (await params).locale;
  if (locale === "en") permanentRedirect("/et/kontakt");
