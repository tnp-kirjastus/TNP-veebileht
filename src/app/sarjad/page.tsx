...
export default async function SeriesPage() {
  const [allSeries, allProducts] = await Promise.all([getSeries(), getAllProducts()]);
  const series = allSeries.map((item) => ({ ...item, products: allProducts.filter((product) => product.series_slug === item.slug).sort(byAvailabilityThenDate) })).filter((item) => item.products.length).sort((a, b) => (b.products[0]?.release_date || "").localeCompare(a.products[0]?.release_date || ""));
