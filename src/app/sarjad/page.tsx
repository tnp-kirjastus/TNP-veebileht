...
export const metadata: Metadata = { title: "Sarjad", description: "Raamatusarjad viimati ilmunud raamatu järgi." };
...
export default function SeriesPage() {
  ...
  return <LayoutFull><section className="py-[50px] border-b border-line"><Shell><Breadcrumbs crumbs={[{ label: "Esileht", href: "/" }, { label: "Sarjad" }]} /><h1 className="font-heading text-[clamp(42px,7vw,78px)] leading-none mt-[18px]">Sarjad</h1><p className="max-w-[620px] mt-4 text-muted">Kõik raamatusarjad on järjestatud sarja viimati ilmunud raamatu alusel. Raamatud viivad toote täpsemasse vaatesse.</p></Shell></section>{series.map((item, index) => <section key={item.slug} className={`py-[42px] border-b border-line ${index % 2 ? "bg-white" : ""}`}><Shell><div className="flex justify-between items-end gap-5 mb-[22px]"><div><h2 className="font-heading text-[30px]">{item.name}</h2><p className="text-muted">{item.products.length} raamatut</p></div><Link href={`/sarjad/${item.slug}`} className="text-accent font-extrabold">Vaata kõiki</Link></div><ProductGrid products={item.products.slice(0, 5).map(card)} columns={5} /></Shell></section>)}<NewsletterSection /></LayoutFull>;
}
