...
import { getSalePercent, isOnSale } from "@/lib/product-utils";
...
export default async function SeriesPage() {
  const [allSeries, activeProducts] = await Promise.all([getSeries(), getActiveProducts()]);
  const series = allSeries.map((item) => ({ ...item, products: activeProducts.filter((product) => product.series_slug === item.slug).sort((a, b) => (b.release_date || "").localeCompare(a.release_date || "")) })).filter((item) => item.products.length).sort((a, b) => (b.products[0]?.release_date || "").localeCompare(a.products[0]?.release_date || ""));
